...
> 🔧 **비유**: v2는 드라이버, 렌치, 플라이어가 각각 따로 있는 공구함이었다면, v3는 **레고 블록** — 같은 블록(프리미티브)을 조합해서 드라이버도, 렌치도, 완전히 새로운 공구도 만들 수 있다.
...
> 🍳 **비유: 레스토랑으로 이해하기**
>
> - **Component** = 메뉴의 각 요리 (파스타, 스테이크, 샐러드)
> - **Provider** = 재료 공급처 (자체 텃밭, 수산시장, 수입업체, 옆 식당 체인)
> - **Transform** = 주방 규칙 (이름표 붙이기, VIP만 제공하는 메뉴, 메뉴판 순서 변경)
>
> v2에서는 "자체 텃밭용 주방", "수입용 주방", "체인점용 주방"이 각각 따로 있었다면,
> v3에서는 **하나의 주방 시스템**으로 모든 재료를 처리한다.

### 2단계 Transform 시스템

> 🏭 **비유: 공장 품질관리 라인**
>
> - **Provider-level Transform** = 각 부품 공급업체의 자체 검수 (A업체는 A 기준으로, B업체는 B 기준으로)
> - **Server-level Transform** = 최종 조립 라인의 통합 품질검사 (모든 부품에 동일한 기준 적용)
>
> 부품이 공급업체에서 나올 때 한 번, 최종 제품에 들어갈 때 한 번 — **2단계 필터링**.
...
> 🏪 **비유**: 해외직구 대행. 해외 쇼핑몰(HTTP 서버)에서 직접 못 사니까, 대행업체(Proxy)가 대신 주문하고 국내 택배(stdio)로 전달해주는 것.
...
> 🏢 **비유**: 회사 건물 로비 보안 데스크. 방문자(Client)가 어떤 부서(Service)를 가든 반드시 로비(Gateway)를 거치며, 신분 확인(인증), 방문 기록(로깅), 출입 제한(정책)이 한 곳에서 처리된다.
...
> 🛒 **비유**: 쿠팡 마켓플레이스. 수백 개 판매자(MCP 서버)가 있지만, 고객(Client)은 쿠팡 앱 하나(Aggregator)만 열면 된다. 검색하면 모든 판매자의 상품이 한 번에 보이고, 주문하면 해당 판매자에게 자동으로 전달된다.
...
> 📞 **비유**: 회사 전화번호 내선번호 시스템. 대표번호(Main Server)로 전화하면, 내선번호 앞자리(namespace)만으로 부서(sub-server)가 자동 연결된다. `1xx`은 영업팀, `2xx`은 개발팀, `3xx`은 인사팀 — 별도 교환원(라우팅 로직) 없이 번호 규칙 자체가 라우팅.
...
> 🎮 **비유**: RPG 게임의 스킬 트리. 처음엔 기본 스킬(공개 도구)만 쓸 수 있다. 레벨업(인증)하면 상위 스킬이 해금되고, 특정 퀘스트(요청)를 완료하면 히든 스킬이 등장한다. 모든 스킬을 처음부터 보여주면 초보자가 혼란스러우니까, **필요할 때 점진적으로 공개**하는 것.
...
> 🏠 **비유: 집 리모델링**
> - Phase 1 = 기존 가구에 커버만 씌우기 (프록시 래핑 — 코드 변경 없음)
> - Phase 2 = 가구들을 한 방에 모으기 (애그리게이터 — 통합)
> - Phase 3 = 인테리어 규칙 적용하기 (Transform — 정책)
> - Phase 4 = 가구를 새로 사기 (네이티브 — 최적 성능)
